<template>
  <div class="py-5">
    <b-container fluid class="my-container-medium py-4">
      <h1 class="text-center w700 h2">{{natriumEULA.title}}</h1>
      <div class="content">
...
<script>
import { getNatriumEULA } from '~/api/posts'
export default {
  layout: 'policy',
  components: {},
  async asyncData() {
    const natriumEULA = await getNatriumEULA()
    return { natriumEULA: natriumEULA }
  },
  data() {
    return {
      homePageDescription:
        'Natrium is owned, operated and released by Appditto. This end user license agreement is a binding agreement between us and our users.',
      homePageTitle: 'Appditto | Natrium End User License Agreement ',
