<template>
  <div class="w-full">
    <div class="container-md px-6 py-12 md:py-20">
      ...
      >{{natriumPrivacyPolicy.title}}</h1>
      <div class="content" v-html="natriumPrivacyPolicy.html"></div>
    </div>
  </div>
</template>
<script>
...
export default {
  ...
  async asyncData() {
    const getSinglePost = async () => {
      try {
        return await axios.get(
          'http://localhost:3000/api/ghost/posts/natrium-mobile-application-privacy-policy'
        )
      } catch (e) {
        console.error(e)
      }
    ...
    const natriumPrivacyPolicy = rawPost.data
