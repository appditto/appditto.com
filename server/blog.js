...
const updateBlogData = async (rawPosts, rawBlogHash) => {
      ...
      if (post.feature_image !== undefined && post.html.match(imgTagRegex)) {
        ...
        post.html.match(imgTagRegex).forEach((imgTag) => {
          ...
          if (imgTag.match(kgImageRegex)) {
            ...
            post.html = post.html.replace(
              imgSrc,
              `src="${cloudinaryUrl1200w + imgUrl}" srcset="${
                cloudinaryUrl100w + imgUrl
              }" data-srcset="${cloudinaryUrl300w + imgUrl} 300w, ${
                cloudinaryUrl600w + imgUrl
              } 600w, ${cloudinaryUrl900w + imgUrl} 900w, ${
                cloudinaryUrl1200w + imgUrl
              } 1200w, ${cloudinaryUrl1500w + imgUrl} 1500w, ${
                cloudinaryUrl2000w + imgUrl
              } 2000w"`
            )
          }
        })
  ...
  editRawPostsAndReturnEdited(rawBlogHash)
    .then((data) => {
      ...
      data.posts.forEach((post) => {
        ...
        fs.writeFile('./blog/posts/' + post.slug + '.json', postData, (err) => {
          if (err) throw err
          console.log(
            `Updated: ${post.slug}, At: ${Date()}, Timestamp: ${Date.now()}`
          )
        })
      ...
      return {
        blogPostsWithoutPolicies: blogPostsWithoutPolicies,
        rawBlogHash: rawBlogHash,
      }
    })
    .then((data) => {
      let blogDataWithoutPolicies = JSON.stringify(
        data.blogPostsWithoutPolicies
      )
      // Update the blog.json file
      fs.writeFile('./blog/blog.json', blogDataWithoutPolicies, (err) => {
        if (err) throw err
        console.log(
          `Updated: blog.json, At: ${Date()}, Timestamp: ${Date.now()}`
        )
      })
      ...
      fs.writeFile(
        './blog/blog-no-html.json',
        JSON.stringify(postsNoHtmlNoPolicy),
        (err) => {
          if (err) throw err
          console.log(
            `Updated: blog-no-html.json, At: ${Date()}, Timestamp: ${Date.now()}`
          )
        }
      )
      // Update the last-three-posts.json file
      fs.writeFile(
        './blog/last-three-posts.json',
        JSON.stringify(lastThreePostsNoHtmlNoPolicy),
        (err) => {
          if (err) throw err
          console.log(
            `Updated: last-three-posts.json, At: ${Date()}, Timestamp: ${Date.now()}`
          )
        }
      )
    ...
    .then((rawBlogHash) => {
      fs.writeFile(
        './blog/last-update.json',
        JSON.stringify([{ timestamp: Date.now(), hash: rawBlogHash }]),
        (err) => {
          if (err) throw err
          console.log(
            `Updated: last-update.json, At: ${Date()}, Timestamp: ${Date.now()}`
          )
        }
      )
    })
